...
// ---------- ברכת הלבנה ----------
// תחילת הזמן: לספרדים ממוצאי היום שבו אירע המולד (שבעים ושתיים שעות אחריו
// לפי מנהגים אחרים; כאן ננקטה הדעה המקדימה), לאשכנזים משבעה ימים מלאים אחרי
// המולד. סוף הזמן לשתי העדות: מחצית הזמן שבין מולד לחודש הבא (מולד + מחצית
// אורך החודש הממוצע, כ-14 יום 18 שעות 22 דקות).
const HALF_MONTH_MS = MONTH_MS / 2;
const SEVEN_DAYS_MS = 7 * 86400000;

// מברכים בלילה בלבד, ולכן תחילת הזמן לספרדים היא הערב שאחרי המולד ולא רגע
// המולד עצמו - שנופל לרוב באמצע היום. הערב מקורב לשעה קבועה (19:00 שעון
// ישראל) ולא לשקיעה אמיתית: העמוד הזה אינו מבקש את מיקום המשתמש לשום דבר
// אחר (ראה ההערה בראש important-dates.js), וחישוב שקיעה היה גורר את כל
// מנוע המיקום בשביל שורה אחת. הכרטיס בעמוד מציין שזו הערכה.
...
function getBirkatHalevanaWindowForMolad(molad) {
  ...
  return {
    molad,
    sephardicStart: eveningAfter(molad),
    ashkenazicStart: new Date(molad.getTime() + SEVEN_DAYS_MS),
